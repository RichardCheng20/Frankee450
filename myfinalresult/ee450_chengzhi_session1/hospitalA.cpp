...
#define HA_PORT "30546"    // the UDP port of hospital A to connect with scheduler
...
//    argv[3] = "8";
    ...
    if ((rv = getaddrinfo(IP_ADDRESS, HA_PORT, &hints, &servinfo)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
    }
    ...
    printf("Hospital A has total capacity %s and initial occupancy %s.\n", argv[2], argv[3]);
    ...
    while (1) {
        ...
        if (strcmp(loc_src, loc_des) == 0) {
            ...
            //not the same, check if src(client) is in map or not
            l_not_in = short_dist(loc_src, loc_des, distance_char, l_not_in);
            //if src(client) is not in the map, need to avoid NONE
            if (strcmp(distance_char, NONE) != 0) {
                distance_num = atof(distance_char);
            }
        }

        if (strcmp(client_loc_char, "A") != 0) {
            //// message 2----------------------------------------
            printf("Hospital A has received input from client at location %s\n", client_loc_char);
            ////for location finding, no location found
            //// message 3----------------------------------------
            if (l_not_in) {
                printf("Hospital A does not have the location %s in map\n", client_loc_char);
            ...
            //// message 5 -----------------------------
            //printf("Hospital A has capacity = %s, occupation= %s, availability = %s\n", argv[2], argv[3], availability);
            printf("Hospital A has capacity = %d, occupation= %d, availability = %s\n", (int)capacity_num, (int)occupation_num, availability);
            //// message 6 -----------------------------
            printf("Hospital A has found the shortest path to client, distance = %s\n", distance_char);
            ...
            //// message 7 -----------------------------
            printf("Hospital A has the score = %s\n", score);
            ...
            //// message 8 -----------------------------
            printf("Hospital A has sent score = %s and distance = %s to the Scheduler\n", score, distance_char);


        } else {
            ////---------------------if Hospital A is chosen!!----------------------
