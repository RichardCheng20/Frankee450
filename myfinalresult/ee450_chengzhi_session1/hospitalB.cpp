 ...
 * dijkstras algorithm is modified fromhttps://www.geeksforgeeks.org/dijkstras-shortest-path-algorithm-greedy-algo-7/
 * Name: Chengzhi Fu
...
#define HB_PORT "31546"    // the UDP port of hospital B to connect with scheduler
...
//    argv[3] = "8";
    ...
    if ((rv = getaddrinfo(IP_ADDRESS, HB_PORT, &hints, &servinfo)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
    }
    ...
    printf("Hospital B has total capacity %s and initial occupancy %s.\n", argv[2], argv[3]);
    ...
    while (1) {
        ...
        if (strcmp(client_loc_char, "B") != 0) {
            ...
            //// message 5 -----------------------------
           //  printf("Hospital B has capacity = %s, occupation= %s, availability = %s\n", argv[2], argv[3], availability);
            printf("Hospital B has capacity = %d, occupation= %d, availability = %s\n", (int)capacity_num, (int)occupation_num, availability);
            //// message 6 -----------------------------
            printf("Hospital B has found the shortest path to client, distance = %s\n", distance_char);
            ...
            //// message 7 -----------------------------
            printf("Hospital B has the score = %s\n", score);
            ...
            //// message 8 -----------------------------
            printf("Hospital B has sent score = %s and distance = %s to the Scheduler\n", score, distance_char);

        } else {
            ////---------------------if Hospital B is chosen!!----------------------
